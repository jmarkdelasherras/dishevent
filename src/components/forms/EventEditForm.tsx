...
interface EventEditFormProps {
  event: RealtimeEventWithId;
  onSubmit: (eventId: string, data: Partial<RealtimeEvent>) => Promise<void>;
  onCancel: () => void;
...
export default function EventEditForm({ 
  ...
  // Initialize the form with event data - only do this once when event is first loaded
  useEffect(() => {
    if (event && event.id) { // Make sure we have a valid event with an ID
      const passwordProtected = event.extraFields?.passwordProtected as boolean || false;
      const password = event.extraFields?.password as string || '';
      const status = (event.status || 'draft') as 'active' | 'draft' | 'completed';
      const planType = (event.planType || 'free') as 'free' | 'premium';
      
      console.log('Initializing form with event data:', event);
      
      reset({
        title: event.name || '', // Map 'name' from RealtimeEvent to 'title' in form, with fallback
        description: event.description || '',
        eventType: event.eventType || 'wedding', // Provide fallback values for all required fields
        date: event.date 
          ? (event.date.includes('T') ? event.date.split('T')[0] : event.date) 
          : new Date().toISOString().split('T')[0],
        maxGuests: typeof event.maxGuests === 'number' ? event.maxGuests : 0,
  ...
  // We're intentionally removing the reset dependency to prevent re-runs
  ...
  const handleFormSubmit = async (data: EventFormData) => {
    ...
    if (data.passwordProtected && data.password) {
      ...
      // If password protection is turned off, remove the password
      eventData.extraFields = {
        ...event.extraFields,
        passwordProtected: false,
        password: '',
  ...
  return (
    <form onSubmit={typedHandleSubmit(handleFormSubmit)} className="space-y-8">
      {/* Form Header */}
      <div className="bg-gradient-to-r from-blue-100 to-green-100 p-5 rounded-lg shadow-sm mb-6">
        ...
        <p className="text-sm text-gray-600 ml-8">Update your event details below</p>
      </div>
